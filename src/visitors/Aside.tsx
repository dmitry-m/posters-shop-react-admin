...
import { Order as OrderRecord, Review as ReviewRecord, Customer, Product } from "../types";
...
const mixOrdersAndReviews = (orders?: OrderRecord[], reviews?: ReviewRecord[]): AsideEvent[] => {
  const eventsFromOrders = orders
    ? orders.map<AsideEvent>((orderItem) => ({
        type: "order",
        date: orderItem.date,
        data: orderItem,
      }))
  ...
  const eventsFromReviews = reviews
    ? reviews.map<AsideEvent>((reviewItem) => ({
        type: "review",
...
const ReviewTitle = () => {
  ...
  const translate = useTranslate();
  const { referenceRecord } = useReference<Product>({
    reference: "products",
